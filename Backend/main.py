...
async def generate_qr(participant: dict):
    ...
    if not os.path.exists(qr_filename):
        ...
        qr_with_id = add_id_to_qr(qr, participant_id)
    ...
    return {"message": f"QR code generated for {participant_id}", "qr_code_url": qr_base64_url, "filename": qr_filename}
...
async def generate_all_qr():
    ...
    for participant in participants:
        if participant["QR_Code"].strip():
            ...
            qr = qrcode.make(qr_data)
            qr_with_id = add_id_to_qr(qr, participant["ID"])
            qr_with_id.save(qr_filename)
    ...
    return {"message": "QR codes generated for all participants", "qr_codes": qr_codes}

@app.post("/scan_qr", response_model=dict)
async def scan_qr(scan_request: ScanRequest):
    ...
    try:
        ...
        print(f"Parsed ID: {participant_id}")  
        if not participant_id:
            raise HTTPException(status_code=400, detail="No ID found in QR data")
    ...
    if participant_data["Snack_Received"] == "Yes":
        print(f"{participant_id} already received snack")  
        return {"message": f"{participant_id} has already received a snack", "updated": False}
    
    participant_data["Snack_Received"] = "Yes"
    print(f"Updated {participant_id} to Snack_Received: Yes")  
    save_participants(participants)
    return {"message": f"Snack marked as received for {participant_id}", "updated": True}